...
# ── BRAND PALETTE ─────────────────────────────────────────────────────────────
C = {
    "maroon":    "#5C0632",   # Qatar Airways primary
    "gold":      "#B8860B",
    "navy":      "#0D2B4E",
    "teal":      "#1D7874",
    "slate":     "#4A5568",
...
def save(fig, name, tight=True):
    path = f"charts/{name}"
    if tight:
        fig.savefig(path, dpi=180, bbox_inches="tight", facecolor=C["cream"])
    else:
        fig.savefig(path, dpi=180, facecolor=C["cream"])
    print(f"  ✓ Saved {path}")
    plt.close(fig)
...
sns.heatmap(heat_data, ax=ax, cmap="RdYlGn", vmin=2.5, vmax=5.0,
...
band_scores.plot(kind="bar", ax=ax, colormap="viridis",
...
sns.heatmap(ct, ax=ax, cmap="Blues", annot=True, fmt="d",
...
sns.heatmap(q_heat, ax=ax, cmap="RdYlGn", vmin=4, vmax=10,
...
kpis = [
    ("1,957",        "Total Reviews",         "2016–2026",   C["navy"]),
    (f"{df['overall_rating_10'].mean():.1f}/10",
                     "Avg Overall Rating",    "Fleet-wide",  C["maroon"]),
    (f"{(df['recommended']=='Yes').mean()*100:.0f}%",
                     "Recommend Rate",        "All cabins",  C["teal"]),
    ("5.8/10",       "2024 Avg Rating",       "Lowest on record", C["red"]),
]
