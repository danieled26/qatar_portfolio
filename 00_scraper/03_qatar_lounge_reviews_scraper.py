...
# CONFIG
# =========================
# BASE_URL = "https://www.airlinequality.com/lounge-reviews/qatar-airways/"
OUTPUT_CSV = "qatar_lounge_reviews.csv"
LOUNGE_URLS = [
    "https://www.airlinequality.com/lounge-reviews/qatar-airways/",
    ]
...
LABEL_MAP = {
    ...
    "airport": "airport",
    "type of lounge": "type_of_lounge",
    "date visit": "date_visit",
    "type of traveller": "type_of_traveller",

    "comfort": "comfort",
    "cleanliness": "cleanliness",
    "bar & beverages": "bar_beverages",
    "catering": "catering",
    "food choices/quality": "catering",
    "food choices / quality": "catering",
    "washrooms": "washrooms",
    "washrooms/showers": "washrooms",
    "washrooms & showers": "washrooms",
    "wifi connectivity": "wifi_connectivity",
    "internet/wifi": "wifi_connectivity",
    "internet / wifi": "wifi_connectivity",
    "staff service": "staff_service",
    "quality of staff service": "staff_service",
...
def derive_trip_month(month_text, review_date_dt):
    """
    Rule:
    - use the review-specific month field if present (e.g. Date Visit)
    - else use review date month
    """
    month_dt = parse_month_year(month_text)
...
def resolve_lounge_mapped_column(label):
...
def parse_review_card(card, page_number, source_url, scrape_order):
    ...
    if table:
        for tr in table.select("tr"):
            ...
            label = (
                label.replace("bar and beverages", "bar & beverages")
                    .replace("food choices / quality", "food choices/quality")
                    .replace("internet/wifi", "wifi connectivity")
                    .replace("internet / wifi", "wifi connectivity")
                    .replace("wifi & connectivity", "wifi connectivity")
                    .replace("washrooms/showers", "washrooms")
                    .replace("washrooms & showers", "washrooms")
                    .replace("quality of staff service", "staff service")
            )
            
            mapped_col = resolve_lounge_mapped_column(label)
            if not mapped_col:
                continue
    ...
    # Derive trip month
    trip_month_dt = derive_trip_month(row["date_visit"], review_date_dt)
