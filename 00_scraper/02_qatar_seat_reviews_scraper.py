...
# CONFIG
# =========================
# BASE_URL = "https://www.airlinequality.com/seat-reviews/qatar-airways"
OUTPUT_CSV = "qatar_seat_reviews.csv"
SEAT_URLS = [
    "https://www.airlinequality.com/seat-reviews/qatar-airways/",
    "https://www.airlinequality.com/seat-reviews/qatar-airways/?airline_overview_score_type=first_business",
    ]
...
COLUMNS = [
    ...
    # business / first style fields
    "sleep_comfort",
    "sitting_comfort",
    "seatbed_width",
    "seatbed_length",
    "seat_privacy",
    "seat_power_supply_firstbusiness",
    "seat_storage_firstbusiness",
...
LABEL_MAP = {
    ...
    # economy / premium style
    "seat legroom": "seat_legroom",
    "seat recline": "seat_recline",
    "seat width": "seat_width",
    "aisle space": "aisle_space",
    "viewing tv screen": "viewing_tv_screen",
    "power supply": "power_supply",
    "seat storage": "seat_storage",

    # business / first style
    "sleep comfort": "sleep_comfort",
    "sitting comfort": "sitting_comfort",
    "seat/bed width": "seatbed_width",
    "seat/bed length": "seatbed_length",
    "seat privacy": "seat_privacy",

    # same visible label, but business/first class html class differs
...
def resolve_seat_mapped_column(header_td, label):
    header_classes = header_td.get("class", [])

    # Business/First variants use different class names in HTML
    if "seat_power_supply_firstbusiness" in header_classes:
        return "seat_power_supply_firstbusiness"

    if "seat_storage_firstbusiness" in header_classes:
        return "seat_storage_firstbusiness"

    # fall back to visible label mapping
...
def build_page_url(base_url, page_num):
    if "?" in base_url:
        root, query = base_url.split("?", 1)
        return f"{root.rstrip('/')}/page/{page_num}/?{query}"
    return f"{base_url.rstrip('/')}/page/{page_num}/"

def fetch_page(session, base_url, page_num):
    page_url = build_page_url(base_url, page_num)
    params = {"pagesize": PAGESIZE}
    sep = "&" if "?" in page_url else "?"
    final_url = page_url + sep + urlencode(params)
...
def parse_review_card(card, page_number, source_url, scrape_order):
    ...
    if title_tag:
        ...
        row["review_title"] = title_text
    else:
        row["review_title"] = DEFAULT_VALUE
    ...
    if table:
        for tr in table.select("tr"):
            ...
            mapped_col = resolve_seat_mapped_column(header_td, label)
            if not mapped_col:
                continue
...
def main():
    ...
    for base_url in SEAT_URLS:
        print(f"\nStarting source: {base_url}")
        page_number = 1

        while True:
            print(f"Scraping page {page_number}...")
            html, resolved_url = fetch_page(session, base_url, page_number)
            soup = BeautifulSoup(html, "html.parser")

            review_cards = soup.select('article[itemprop="review"]')

            if not review_cards:
                print("No review cards found. Stopping this source.")
                break
            ...
            if not page_review_dates:
                print("No parseable review dates found on page. Stopping this source.")
                break

            oldest_review_date_on_page = min(page_review_dates)

            if oldest_review_date_on_page < REVIEW_STOP_DATE:
                print("Reached review-date lower bound for this source. Stopping pagination.")
                break
